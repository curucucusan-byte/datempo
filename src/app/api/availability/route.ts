...
import { getProfessional, resolveProfessional, getServiceMinutes } from "@/lib/professionals";
import { loadAppointments } from "@/lib/store";
import { getAccount } from "@/lib/account";
import { freeBusyForDate } from "@/lib/google";
...
export async function GET(req: Request) {
  ...
  if (!professional) {
    return NextResponse.json<ErrorResult>({ ok: false, error: "Profissional não encontrado." }, { status: 404 });
  }

  const fallbackMinutes = professional.services?.[0]?.minutes ?? 60;
  const slotMinutes = Math.max(
    5,
    getServiceMinutes(professional, service ?? "", fallbackMinutes)
  );

  // Definição de slots base: preferimos Google Calendar. Quando indisponível, usamos janela 08:00–20:00.
  const ownerUid = professional.ownerUid ?? null;
  let googleBusy: { start: string; end: string }[] | null = null;
  let daySlots: string[] = [];
  if (ownerUid) {
    const account = await getAccount(ownerUid);
    const calId = (professional as any).calendarId ?? account?.activeCalendarId ?? null;
    if (calId) {
  ...
  if (googleBusy) {
    ...
    const weekday = WEEKDAYS[parsedDate.utcDate.getUTCDay()];
    daySlots = professional.workHours?.[weekday as WeekdayKey] ?? [];
    if (!daySlots.length) {
      // fallback simples 09:00–18:00, caso sem Google e sem horários
      for (let h = 9; h < 18; h++) {
        const hh = String(h).padStart(2, "0");
        daySlots.push(`${hh}:00`);
